  ...
  (async () => {
    ...
    if (!googleUsageFromDB) {
      (client.channels.cache.get('') as TextChannel).send({
        embeds: [
          new MessageEmbed()
            .setDescription("Could not load Google's usage from database.")
            .setColor('BLUE'),
        ],
      });
      return;
  ...
  if (commandName === 'deepl-usage') {
    (async () => {
      ...
      if (usage.character) {
        interaction.reply({
          embeds: [
            EmbedUtils.createTranslatorUsageEmbed(
              'DeepL Translator',
      ...
      if (usage === null || usage === undefined) {
        interaction.reply({
          embeds: [
            new MessageEmbed()
              .setDescription(
                "Could not fetch usage on Google's Translation API! Check the logs."
              )
              .setColor('BLUE'),
          ],
        });
...
client.on('messageCreate', (message) => {
  ...
  englishChannel = client.channels.cache.get(
    guildTranslate.channelsId.english
  ) as TextChannel;
  chineseChannel = client.channels.cache.get(
    guildTranslate.channelsId.chinese
  ) as TextChannel;
  koreanChannel = client.channels.cache.get(
    guildTranslate.channelsId.korean
  ) as TextChannel;
...
function sendTranslationToChannel(
  ...
  if (translatedText) {
    (async () => {
      const embed = await EmbedUtils.createTranslatedMessageEmbed(
        message,
        translatedText,
        sourceChannelName,
        translator
...
async function deeplTranslate(
  ...
  try {
    let translation = '';
    const processedMessage = TranslationUtils.prepareForTranslation(
      message.content
    );
    ...
    if (message.content) {
      ...
      if (!usage.character) {
        message.channel.send({
          embeds: [
            new MessageEmbed()
              .setDescription(
                'Could not check if there are still available characters on DeepL translation service! Check the logs.'
              )
              .setColor('BLUE'),
          ],
        });
      ...
      if (
        usage.character.count + message.content.length >
        usage.character.limit
      ) {
        message.channel.send({
          embeds: [
            new MessageEmbed()
              .setDescription(
                "Uh oh! Cannot translate anymore using DeepL's translation API. The limit will be reached!"
              )
              .setColor('BLUE'),
          ],
        });
    ...
    sendTranslationToChannel(
      message,
      destinationChannel,
      TranslationUtils.returnEmojisToTranslation(
        translation,
        processedMessage.emojiTable
      ),
      sourceChannelName,
      'DeepL'
    );
    ...
    message.channel.send({
      embeds: [
        new MessageEmbed()
          .setDescription(
            "An error has occurred while processing translation using DeepL's translation API! Check the logs."
          )
          .setColor('BLUE'),
      ],
    });
...
async function googleTranslate(
  ...
  try {
    ...
    if (message.content) {
      if (
        googleUsage.characterUsed + message.content.length >
        googleUsage.characterLimit
      ) {
        message.channel.send({
          embeds: [
            new MessageEmbed()
              .setDescription(
                "Uh oh! Cannot translate anymore using Google's Cloud Translate API. The limit will be reached!"
              )
              .setColor('BLUE'),
          ],
        });
      ...
      const [translation] = await googleTranslator.translate(
        processedMessage.processedText,
        targetLang
      );
      processedTranslation = TranslationUtils.returnEmojisToTranslation(
        translation,
        processedMessage.emojiTable
      );
    }
    ...
    mongoClient
      .db('Zekuru')
      .collection('GoogleTranslateUsageEntries')
      .insertOne({
    ...
    if (usage === null || usage === undefined) {
      message.channel.send({
        embeds: [
          new MessageEmbed()
            .setDescription(
              "Could not check if there are still available characters on Google's Cloud Translate API! Check the logs."
            )
            .setColor('BLUE'),
        ],
      });
    ...
    message.channel.send({
      embeds: [
        new MessageEmbed()
          .setDescription(
            "An error has occurred while processing translation using Google's Cloud Translation API! Check the logs."
          )
          .setColor('BLUE'),
      ],
    });
